...
# make sure sub_id and obj_id are between 0-4
if sub_id < 0 or sub_id > 4:
    print('[ERROR] Please enter a valid subject id!')
    exit()
if obj_id < 0 or obj_id > 4:
    print('[ERROR] Please enter a valid object id!')
    exit()

sub_addr = subject_address_list[sub_id]
obj_addr = object_address_list[obj_id]
...
try:
    # Build transaction
    txn = subject_contract.functions.change_attribs(sub_addr, attrib_list).buildTransaction(tr)
...
nonce = w3.eth.get_transaction_count(sub_addr)
tr = {
    'from': sub_addr,
    'gasPrice': Web3.toHex(gas_price),
    'nonce': Web3.toHex(nonce)
...
# Build transaction
txn = access_contract.functions.access_control(obj_addr, action).buildTransaction(tr)
# Sign the transaction
signed = w3.eth.account.sign_transaction(txn, subject_dict[sub_addr])
# Send raw transaction
tx_hash = w3.eth.send_raw_transaction(signed.rawTransaction)
# wait for the transaction to be mined and get the transaction receipt
tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
