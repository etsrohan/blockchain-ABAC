...
object_contract = w3.eth.contract(address=object_address, abi=object_abi)

# Connect to subject contract
subject_contract = w3.eth.contract(address=subject_address, abi=subject_abi)
...
async def pol_loop(event_filter, poll_interval):
    ...
    while True:
        for new_pol_added in event_filter.get_new_entries():
            thread = threading.Thread(
                target=handle_new_pol, args=(new_pol_added["args"]["pol_id"],)
            )
            thread.start()
        await asyncio.sleep(poll_interval)
...
async def auth_success_loop(event_filter, poll_interval):
    ...
    while True:
        for success in event_filter.get_new_entries():
            thread = threading.Thread(
                target=handle_auth_succ, args=(success["args"]["sub_addr"],)
            )
            thread.start()
        await asyncio.sleep(poll_interval)
...
async def auth_fail_loop(event_filter, poll_interval):
    ...
    while True:
        for failure in event_filter.get_new_entries():
            thread = threading.Thread(
                target=handle_auth_fail, args=(failure["args"]["sub_addr"],)
            )
            thread.start()
        await asyncio.sleep(poll_interval)
...
async def sub_change_loop(event_filter, poll_interval):
    ...
    while True:
        for change in event_filter.get_new_entries():
            thread = threading.Thread(
                target=handle_sub_change, args=(change["args"]["sub_addr"],)
            )
            thread.start()
        await asyncio.sleep(poll_interval)
...
# Main Function
def main():
    subject_filter = subject_contract.events.NewSubjectAdded().createFilter(
        fromBlock="latest"
    )
    sub_change_filter = subject_contract.events.SubjectChanged().createFilter(
        fromBlock="latest"
    )
    object_filter = object_contract.events.NewObjectAdded().createFilter(
        fromBlock="latest"
    )
    policy_filter = policy_contract.events.PolicyAdded().createFilter(
        fromBlock="latest"
    )
    access_success = access_contract.events.AccessGranted().createFilter(
        fromBlock="latest"
    )
    access_failure = access_contract.events.AccessDenied().createFilter(
        fromBlock="latest"
    )
    authentication_success = (
        access_contract.events.AuthenticationSuccess().createFilter(fromBlock="latest")
    )
    authentication_failure = (
        access_contract.events.AuthenticationFailure().createFilter(fromBlock="latest")
    )
    transfer_filter = token_contract.events.AdminTransfer().createFilter(
        fromBlock="latest"
    )
